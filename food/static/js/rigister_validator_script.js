...
// site name : foodino
// file name : register_validator_script.js
...
function password_checker() {
  ...
  if (value.length<=8 || value.length >= 16) {
    ...
    if (password_flag === true && re_password_flag === true && mobile_number_flag === true) {
...
function re_password_checker() {
  ...
  error_box_message.classList.replace('hidden','show')
    if (value.length == 0){
      ...
      re_password_flag = false
      error_box_message.innerText="رمز عبور مطابقت ندارد."
      navigator.vibrate([100, 200,100]);
      input_submit_element.disabled = true;
  }
  else{
...
document.querySelector('input[name="mobile_number"]').addEventListener('input',function(){mobile_number_checker()})
